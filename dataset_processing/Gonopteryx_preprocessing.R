#Initial processing Gonepteryx data for data analysis
...
transcript.df <- read.table(text=transcript.df[which(!transcript.df$record%in%tolower(" goma11_3_20220516_145652_MM")),], col.names="record", sep = ",") # Remove a recording transcribed twice (by MM and MB)
...
write.csv(gonepteryx.trials, "tested_specimens_matched.xlsx")
...
#Check to see is there any residual Gonepteryx (not wanted for gonepteryx)
grep("Le",dfiles1)
...
#Check if names in table matches file name
#files[which(as.vector(sapply(data, function(x) any(tolower(x$id)%in%"goma07"))))]
...
# Chek recording lasting than 20 minutes and without oviposition
nnn <- do.call(rbind.data.frame,lapply(lapply(data2, tail,1), function(x) cbind(x$activity_time-x$start_time, x$behaviour)))
nnn$V1 <- as.numeric(nnn$V1)
nnn[which(nnn$V1<20&!nnn$V2%in%"ov"),]
...
gonagg <- do.call(rbind.data.frame, data3)
gonagg$duration <- as.integer(gonagg$duration)
...
df.gonepteryx <- merge(gonagg, gonepteryx.trials[,c("type","transcript")], by="transcript", all.x=T)
...
# Here the final dataset for gonepteryx
df.gonepteryx2 <- merge(df.gonepteryx, trr[,-c(2)], by=c("id","start_time"),all.x=TRUE)

# Add a column for proportional time for each behaviour relatively to total test duration
df.gonepteryx2 <- merge(df.gonepteryx2, aggregate(duration~id+n_trial, df.gonepteryx2, "sum"), by=c("id","n_trial"))
names(df.gonepteryx2)[10] <-"duration"
names(df.gonepteryx2)[12] <-"duration_test"
df.gonepteryx2$prop_duration <- df.gonepteryx2$duration/df.gonepteryx2$duration_test

# Redundat check for non allowed durations
df.gonepteryx2[which(df.gonepteryx2$duration_test>1201),]

df.gonepteryx3 <- df.gonepteryx2[order(df.gonepteryx2$id, df.gonepteryx2$n_trial,df.gonepteryx2$cumtime),]
# Order properly
df.gonepteryx3 <- df.gonepteryx3[order(df.gonepteryx3$id, df.gonepteryx3$n_trial,df.gonepteryx3$cumtime),]
head(df.gonepteryx3)

# Here the final dataset for Leptidea
saveRDS(df.gonepteryx3,"df.gonepteryx.RDS")

# Plot durations aggregated per behavioural category
ggplot(df.gonepteryx3, aes(x=behaviour, y=c(duration)/sum(duration)*100)) +
  geom_col() +
  ylab("% of total time") +
  ggtitle(paste("# of Tests:",length(data),"# of Individuals:", length(unique(gonagg$id))))
